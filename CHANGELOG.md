...
- 新增 `tests/picker_unit.php`（headless）：点语言段弹列表 / 初始高亮落在当前值 / ↑↓ 环形 / Enter 应用（语言真的变）/ Esc 只关列表不退出 / **点不可点的段无反应** / 鼠标点列表项应用、点别处关闭 / 主题列表 / 浮层真的画在屏幕上且底边紧贴状态栏上一行 / 关闭时屏幕上没有它（阴性对照）。**五条反向注入全部实测可 FAIL**：段不声明 pick、`clickSegment` 不认 pick、Esc 不消费、浮层下移一格压住状态栏、鼠标命中行算错一格。⚠️ 写这条时踩了两个坑：`findInGrid` 用 `strlen()` 当**格数**（多字节字符永远匹配不上）；以及「注入后变红」**也得确认是断言失败而不是崩溃** —— 第一次的「浮层位置」注入其实把面板挤出了视口、php-tui 直接抛 `Position` 越界，红得毫无意义（已换成界内注入，并加了一条从渲染帧量底边框行的断言）。
