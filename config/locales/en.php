<?php
declare(strict_types=1);

/**
 * 英文语言包。所有界面字符串都从这里取 key。
 * zh_CN 包只需覆盖需要翻译的项；缺失的自动回退到此文件。
 */
return [
    'app.title'        => 'ViceCode',
    'app.stale_exit'   => '⚠ The previous session did not exit cleanly (no fatal error recorded) — see the fatal log',

    // 面板标题
    'panel.sidebar'    => 'SIDEBAR',
    'panel.editor'     => 'EDITOR',
    'panel.terminal'   => 'TERMINAL',
    'panel.ai_chat'    => 'AI CHAT',
    'panel.ai_input'   => 'AI INPUT',

    // 侧栏 tab
    'sidebar.explorer' => 'EXPLORER',
    'sidebar.git'      => 'GIT',
    'sidebar.search'   => 'SEARCH',
    'sidebar.plugins'  => 'EXTENSIONS',

    // 状态栏
    'status.focus'     => 'focus',
    'status.tab'       => 'tab',
    'status.file'      => 'file',
    'status.dirty'     => '*',
    'status.cursors'   => '{n} cursors',
    'status.readonly'  => 'RO',
    'status.locale'    => 'lang',
    // Titles of the option lists opened from clickable status segments (language / theme)
    'picker.locale'    => 'Select language',
    'picker.theme'     => 'Select theme',
    'status.branch'    => 'branch',
    'status.cwd'       => 'cwd',
    'status.provider'  => 'AI',
    'status.no_tools'  => 'no tools',
    'status.ai_unset'  => 'none',
    'status.strategy_auto' => 'auto',
    'status.strategy'  => 'strategy',
    'status.strategy_offpeak' => 'off-peak',
    'status.theme'     => 'Theme',
    'status.mode'      => 'Mode',
    'status.mode_edit' => 'Edit',
    'status.quit'      => 'Ctrl+Q quit',
    'status.l_sidebar' => 'side',
    'status.l_ai'      => 'AI',
    'status.l_editor'  => 'edit',
    'status.l_input'   => 'input',
    // Panel visibility / terminal maximize (B16): appended to the layout summary only when active
    'status.hidden'    => 'hidden',
    'status.hidden_sidebar' => 'sidebar',
    'status.hidden_ai' => 'AI',
    'status.hidden_terminal' => 'terminal',
    'status.term_max'  => 'term max',
    'status.copied'     => 'Copied {n} chars',
    'status.copied_msg' => 'Copied message',
    'status.pasted'      => 'Pasted {n} chars',
    'status.paste_pending' => 'Reading clipboard…',

    // AI 输入框提示
    'ai.input_hint'    => '(Enter send)',

    // 编辑器占位 / 提示
    'editor.no_file'   => '(no file open — pick a file in Explorer, Enter to open)',
    'editor.readonly'  => '(read-only)',
    'editor.too_large' => 'File too large to edit: {size} bytes (limit {limit})',
    'editor.binary'    => 'Binary file — not editable',
    'editor.missing'    => 'File does not exist: {path}',
    'editor.unreadable' => 'No read permission: {path}',
    'editor.saved'     => 'Saved',
    'editor.save_failed' => 'Save failed: {msg}',
    'editor.save_no_permission' => 'Save failed: no write permission or path not writable',
    'editor.open'      => 'Open',
    'editor.untitled'  => 'untitled',

    'confirm.quit_dirty'  => 'Unsaved changes: y quit / n cancel',
    'confirm.close_dirty' => 'File not saved: y close & discard / n cancel',
    'confirm.discard'     => 'Discard working changes (IRREVERSIBLE!): y discard / n cancel: {path}',

    // 终端（M2）
    'term.running'       => 'running',
    'term.exit'          => 'exit code {code}',
    'term.killed'        => 'interrupted (SIGKILL)',
    'term.busy'          => 'A command is running — wait for it or press Ctrl+C to interrupt',
    'term.spawn_failed'  => 'Failed to spawn command process',
    'term.empty'         => '(focus this panel to type a command, Enter to run; ↑/↓ history, Ctrl+L clear)',
    'term.interactive'   => 'Interactive',
    'term.captured'      => 'captured',
    'term.interactive_enter' => 'Type here to take over the keyboard (Esc quits)',
    'term.focus_to_start'    => '(focus this panel to start an interactive shell)',
    'term.interactive_exit'  => 'Interactive terminal exited · F2 to re-enter',
    'term.interactive_hint'  => 'Interactive · F2 to exit capture',

    // 侧栏其它标签占位
    'git.not_repo'       => '(not a git repository)',
    'git.view_status'    => 'status {n}',
    'git.view_log'       => 'log {n}',
    'git.loading'        => '(loading…)',
    'git.clean'          => '(working tree clean)',
    'git.no_log'         => '(no commit history)',
    'git.no_diff'        => 'no diff for this file',
    'git.commit_title'   => 'Commit message',
    'git.msg_placeholder'=> 'Type commit message…',
    'git.changes'        => 'Changes {n}',
    'git.log_title'      => 'Log {n}',
    'git.stage_all'      => 'Staged all changes',
    'git.stage_file'     => 'Staged: {path}',
    'git.unstage_file'   => 'Unstaged: {path}',
    'git.unstage_all'    => 'Unstaged all',
    'git.discard_file'   => 'Discarded working changes: {path}',
    'git.committed'      => 'Committed',
    'git.commit_empty'   => 'Empty message, aborted',
    'git.push_done'      => 'Pushed',
    'git.push_fail'      => 'Push failed: {msg}',
    'git.op_fail'        => 'Operation failed: {msg}',
    'git.branch_pick'    => 'Switch branch',
    'git.no_branch'      => '(no other branch)',
    'git.switch_done'    => 'Switched to branch: {branch}',
    'git.switch_fail'    => 'Switch branch failed: {msg}',
    'git.action_commit'       => 'Commit',
    'git.action_commit_all'   => 'Commit changes',
    'git.action_commit_push'  => 'Commit & push',
    'git.action_commit_sync'  => 'Commit & sync',
    'git.hint'           => '[a]stage all [s]stage [c]commit [p]push [L]log [Enter]diff [R]refresh',
    'search.placeholder'      => 'Search file contents…',
    'search.hint'             => 'Press Enter to search…',
    'search.prompt'           => ' > ',
    'search.status_searching' => 'Searching…',
    'search.status_no_results' => 'No matches',
    'search.results'          => '{n} matches / {files} files',
    'search.status_error'     => 'Search error: {msg}',
    'search.truncated'        => 'Truncated (first {n}), narrow your query',
    'search.empty_query'      => 'Type a search keyword',

    'ai.no_provider'     => 'No provider configured',
    'ai.no_key'          => 'Missing API key: set env {env}',
    'ai.no_selection'    => 'No model selected · Ctrl+P provider, Ctrl+N model',
    'ai.caps'            => 'Capabilities',
    'ai.caps_none'       => 'none declared',
    'ai.streaming'       => 'Generating…',
    'ai.cancelled'       => 'Generation stopped',
    'ai.max_steps'       => 'Reached max tool-call steps ({steps}), stopping',
    'ai.await_approval'  => 'AI requests tool execution, y approve / n deny',
    'ai.attach_missing' => '@{name}: file not found or outside project root, skipped',
    'ai.attached' => 'Attached {n} bytes of code to input',
    'ai.attach_empty' => 'Empty selection, nothing attached',
    'ai.attach_no_selection' => 'Drag-select text in the editor first',
    'ai.attach_no_file' => 'No file is open',
    'menu.ai' => 'AI',
    'ai.act.explain' => 'Explain this code',
    'ai.act.comment' => 'Add comments to this code',
    'ai.act.refactor' => 'Suggest refactoring',
    'ai.act.unittest' => 'Write unit tests',
    'ai.attach_selection_label' => 'Attach editor selection',
    'ai.attach_file_label' => 'Attach current file',
    'ai.tool_mode_confirm' => 'Tool mode: switch to confirm each',
    'ai.tool_mode_auto' => 'Tool mode: switch to auto-run',
    'ai.compact_now_label' => 'Compact conversation now',
    'ai.providers_label' => 'Edit model config…',
    'ai.providers_reloaded' => 'Model config reloaded',
    'ai.providers_bad' => 'Model config ignored ({kind}); previous config kept',
    'ai.strategy_auto_item' => 'Auto (route by task type)',
    'ai.strategy_auto_on' => 'Back to auto routing: tier is picked by task type (quick actions and /prefix)',
    'ai.strategy_applied_pinned' => 'Strategy "{strategy}" → {provider}/{model} (pinned; auto routing paused)',
    'ai.kind_unknown' => 'Unknown task type "/{kind}" (known: {known}) — not sent; use // for a literal slash',
    'ai.kind_ignored_pinned' => 'This message carries task type /{kind}, but you pinned a tier manually, so auto routing was skipped (Ctrl+R cycles back to auto)',
    'ai.strategy_item' => 'Strategy: {label}',
    'ai.strategy_applied' => 'Strategy "{strategy}" → {provider}/{model}',
    'ai.strategy_unknown' => 'No such strategy: {strategy}',
    'ai.strategy_no_provider' => 'Strategy "{strategy}" targets provider "{provider}", which is not configured',
    'ai.strategy_no_model' => 'Strategy "{strategy}" wants model "{model}", not declared by provider "{provider}" (refusing to silently fall back)',
    'ai.strategy_missing_caps' => 'Strategy "{strategy}" requires {caps}, but {model} does not declare it — switch refused (that capability would silently stop working)',
    'ai.strategy_none' => 'No model strategies configured (add an @strategies section in your model config file)',
    'ai.strategy_offpeak_item' => 'off-peak',
    'ai.strategy_offpeak_applied' => 'Off-peak: switched to strategy "{strategy}" → {provider}/{model} (discount until {until})',
    'ai.strategy_offpeak_all_day' => 'all day',
    'ai.cleared_menu' => 'Clear conversation',
    'ai.cleared' => 'Conversation cleared',
    'ai.compact_started' => 'Compacting conversation history…',
    'ai.compact_failed' => 'Compact failed, history kept',
    'ai.compact_done' => 'Compact done ({before} → {after} messages)',
    'ai.compact_instruction' => "Summarize the conversation history below into a concise digest that can serve as background for later turns.\nKeep: the user's intent and constraints, conclusions and decisions reached, file paths and key code locations, and anything unfinished. Drop: pleasantries and repetition. Output only the digest, do not restate this instruction.\n\n===== CONVERSATION HISTORY TO COMPACT =====\n{history}",
    'help.e_ai_explain' => 'AI explain code (in editor)',
    'ai.error'           => 'AI request failed: {msg}',
    'ai.http_error'      => 'HTTP {code}',
    'ai.empty'           => '(model returned nothing)',
    'ai.busy'            => 'Already generating, cannot start a new request',
    'ai.switched'        => 'Switched to {provider} / {model}',
    'ai.clear_hint'      => '(Ctrl+L clears the conversation)',

    // ── M6 R2 帮助页 ─────────────────────────────
    'help.group_global'    => 'Global',
    'help.group_editor'    => 'Editor',
    'help.group_terminal'  => 'Terminal',
    'help.group_explorer'  => 'Explorer',
    'help.group_git'       => 'GIT',
    'help.group_search'    => 'Search',
    'help.group_ai'        => 'AI Chat',
    'help.g_quit'          => 'Quit (confirm if unsaved)',
    'help.g_tab'           => 'Cycle focus; in inputs/editor: indent 4 spaces, accept completion when candidates show',
    'help.g_tab_back'      => 'Cycle focus backwards; in inputs/editor: outdent, previous candidate',
    'help.g_add_cursor'    => 'Add an editing cursor above/below (edit multiple lines at once); Esc collapses them',
    'help.g_add_cursor_click' => 'Add an editing cursor at the click (no selection; without Alt it is a normal drag-copy)',
    'help.g_esc'           => 'Quit; panels may handle it first',
    'help.g_q'             => 'Quit when not typing',
    'help.g_help'          => 'Toggle this help',
    'help.g_menu'           => 'Open the top menu bar',
    'help.g_theme'           => 'Switch color theme',
    'help.g_wheel'         => 'Scroll current panel',
    'help.g_wheel_h'       => 'Scroll current panel horizontally',
    'help.g_click'         => 'Change focus / select item',
    'help.e_move'          => 'Move cursor (wraps across lines)',
    'help.e_home_end'      => 'Line start / end',
    'help.e_page'          => 'Page up / down',
    'help.e_indent'        => 'Indent 4 spaces / outdent (applies to every cursor when there are several)',
    'help.e_newline'       => 'New line',
    'help.e_del'           => 'Delete backward / forward',
    'help.e_save'          => 'Save',
    'help.e_close'         => 'Close current file',
    'help.e_switch'        => 'Switch between open files',
    'help.e_type'          => 'Insert character',
    'help.e_autopair'      => 'Auto-pair: insert closer, skip existing, delete empty pair, wrap selection',
    'help.t_run'           => 'Run command',
    'help.t_history'       => 'Command history',
    'help.t_move'          => 'Move cursor',
    'help.t_home_end'      => 'Line start / end',
    'help.t_del'           => 'Delete backward / forward',
    'help.t_page'          => 'Scroll output',
    'help.t_clear'         => 'Clear screen',
    'help.t_cancel'        => 'Interrupt running command',
    'help.t_interactive'   => 'F2 toggle terminal keyboard capture (interactive shell is the default)',
    'help.x_move'          => 'Move selection',
    'help.x_nav'           => 'Expand (→ into child) / collapse (← to parent)',
    'help.x_enter'         => 'Expand/collapse dir or open file',
    'help.x_open'          => 'Open file',
    'help.x_arrow'         => 'Expand / collapse',
    'help.x_dblclick'      => 'Expand / collapse',
    'help.gi_type'         => 'Type commit message',
    'help.gi_commit'       => 'Commit',
    'help.gi_backspace'    => 'Delete last char of message',
    'help.gi_move'         => 'Move selection',
    'help.gi_stage'        => 'Stage selected file',
    'help.gi_unstage'      => 'Unstage selected file',
    'help.gi_esc'          => 'Close dropdown',
    'help.gi_icons'        => 'Stage / unstage / discard changes / view diff',
    'help.s_type'          => 'Type search keyword',
    'help.s_enter'         => 'Search / open selected hit',
    'help.s_backspace'     => 'Delete last char of keyword',
    'help.s_move'          => 'Move between results',
    'help.a_type'          => 'Type message',
    'help.a_at_ref'        => 'Reference a project file (expanded into a highlighted code block before sending)',
    'help.a_complete'      => 'Accept completion / previous candidate; indents 4 spaces when there is none',
    'help.a_send'          => 'Send',
    'help.a_provider'      => 'Switch provider',
    'help.a_model'         => 'Switch model',
    'help.a_strategy'      => 'Switch model strategy',
    'help.a_clear'         => 'Clear conversation',
    'help.a_history'       => 'Recall prompt history',
    'help.a_page'          => 'Scroll message stream',
    'help.a_stop'          => 'Stop generating',
    'help.e_paste'         => 'Paste (Ctrl+V / Shift+Ins)',
    'help.title'           => 'Keyboard Shortcuts',
    'help.close'           => 'Esc / ? / q to close',
    'help.scroll_hint'     => 'Up/Down or wheel to scroll',

    // ── 顶部菜单栏（Backlog 接入）────────────────
    'menu.file'              => 'File',
    'menu.file_open'         => 'Open',
    'menu.file_save'         => 'Save',
    'menu.file_close'        => 'Close',
    'menu.file_quit'         => 'Quit',
    'menu.view'              => 'View',
    'menu.view_theme'        => 'Switch Theme',
    'menu.view_focus_editor' => 'Focus → Editor',
    'menu.view_focus_terminal' => 'Focus → Terminal',
    'menu.view_focus_explorer' => 'Focus → Explorer',
    'menu.view_focus_ai'     => 'Focus → AI Chat',
    'menu.view_lang'         => 'Switch Language',
    'menu.view_sidebar_hide' => 'Hide Sidebar',
    'menu.view_sidebar_show' => 'Show Sidebar',
    'menu.view_ai_hide'      => 'Hide AI Column',
    'menu.view_ai_show'      => 'Show AI Column',
    'menu.view_term_hide'    => 'Hide Terminal',
    'menu.view_term_show'    => 'Show Terminal',
    'menu.view_term_max'     => 'Maximize Terminal',
    'menu.view_term_restore' => 'Restore Terminal',
    'menu.terminal'          => 'Terminal',
    'menu.term_cancel'       => 'Interrupt',
    'menu.term_clear'        => 'Clear',
    'menu.help'              => 'Help',
    'menu.help_shortcuts'    => 'Shortcuts',
    'menu.help_about'        => 'About',
    'menu.file_plugins'      => 'Installed Plugins...',
    'menu.plugins' => 'Plugins',
    'plugins.commands' => 'Commands',
    'plugins.command_failed' => 'Plugin {plugin} command {cmd} failed',
    'plugins.shortcut_reserved' => 'Shortcut {key} conflicts with a system keybinding; ignored (still available from the Plugins menu)',
    'plugins.shortcut_taken' => 'Shortcut {key} already taken by {owner}; ignored',
    'plugins.shortcut_unsupported' => 'Shortcut {key} unsupported (only Ctrl+letter and F1–F12)',
    'plugins.no_executor' => 'Plugin declares commands but has no executeCommand(); commands disabled',
    'plugins.command_unavailable' => 'Command unavailable ({reason})',
    'plugins.title'          => 'Installed Plugins',
    'plugins.enabled'        => 'enabled',
    'plugins.disabled'       => 'disabled',
    'plugins.toggled_on'     => 'Plugin {id} enabled',
    'plugins.toggled_off'    => 'Plugin {id} disabled',
    'plugins.no_config'      => 'no configurable options',
    'plugins.no_plugins'     => '(no plugins)',
    'plugins.config_path'    => 'Plugin config file',
    'plugins.hint'           => "Space: enable/disable the selected plugin · Enter: edit that file in ViceCode's own editor (Ctrl+S applies live)",
    'plugins.close'          => 'Esc / q to close',
    'plugins.reloaded'       => 'Plugin config reloaded',
    'about.title'            => 'About ViceCode',
    'about.line1'            => 'A terminal IDE built on php-tui',
    'about.line2'            => 'Swoole coroutine driven · multi-provider LLM',
    'about.line3'            => 'F10 menu · ? shortcuts · Ctrl+Q quit',
    'status.lang'            => 'Lang',

    // Theme names (status bar / menu)
    'theme.dark'             => 'Dark',
    'theme.midnight'         => 'Midnight',

    // Help page "trigger" column (mouse / input descriptors)
    'keys.wheel'             => 'Wheel',
    'keys.hwheel'            => 'Horizontal wheel',
    'keys.click'             => 'Click',
    'keys.alt_click'         => 'Alt+click',
    'keys.printable'         => 'Printable char',
    'keys.click_file'        => 'Click file',
    'keys.click_triangle'    => 'Click row triangle',
    'keys.dblclick_dir'      => 'Double-click dir',
    'keys.click_icon'        => 'Click icon',

    // Command palette (F1)
    'palette.title'          => 'Command Palette',
    'palette.placeholder'    => 'Type to filter commands…',
    'menu.command_palette'   => 'Command Palette',
    'help.g_palette'         => 'Open command palette (F1)',

    // Custom plugin panels (overlay host)
    'menu.plugin_panels'     => 'Plugin Panels',
    'panel.title'            => 'Plugin Panels',
    'panel.empty'            => '(No plugin panels)',
    'panel.close'            => 'Esc close · Tab switch',

    // Model capabilities (config/providers.php `capabilities`; unknown ones shown verbatim)
    'cap.tools'              => 'tools',
    'cap.reasoning'          => 'reasoning',
    'cap.vision'             => 'vision',
    'cap.audio'              => 'audio',
];
