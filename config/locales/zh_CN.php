<?php
declare(strict_types=1);

/**
 * 简体中文语言包。只覆盖需要翻译的项；缺失 key 由 Translator 回退到 en.php。
 */
return [
    'app.title'        => 'ViceCode',
    'app.stale_exit'   => '⚠ 上次会话未正常结束（且无致命错误记录），详见致命日志',

    'panel.sidebar'    => '侧栏',
    'panel.editor'     => '编辑器',
    'panel.terminal'   => '终端',
    'panel.ai_chat'    => 'AI 对话',
    'panel.ai_input'   => 'AI 输入',

    'sidebar.explorer' => '资源管理器',
    'sidebar.git'      => 'GIT',
    'sidebar.search'   => '搜索',
    'sidebar.plugins'  => '扩展',

    'status.focus'     => '焦点',
    'status.tab'       => '标签',
    'status.file'      => '文件',
    'status.dirty'     => '*',
    'status.cursors'   => '{n} 个光标',
    'status.readonly'  => '只读',
    'status.locale'    => '语言',
    // 状态栏可点段弹出的选项列表标题（语言 / 主题）
    'picker.locale'    => '选择语言',
    'picker.theme'     => '选择主题',
    'status.branch'    => '分支',
    'status.cwd'       => '目录',
    'status.provider'  => 'AI',
    'status.no_tools'  => '无工具',
    'status.ai_unset'  => '未选',
    'status.strategy_auto' => '自动',
    'status.strategy'  => '策略',
    'status.strategy_offpeak' => '折扣',
    'status.theme'     => '主题',
    'status.mode'      => '模式',
    'status.mode_edit' => '编辑',
    'status.quit'      => 'Ctrl+Q 退出',
    'status.l_sidebar' => '侧栏',
    'status.l_ai'      => 'AI',
    'status.l_editor'  => '编辑',
    'status.l_input'   => '输入',
    // 面板显隐 / 终端最大化（B16）：仅在有隐藏/最大化时追加到布局摘要里
    'status.hidden'    => '隐藏',
    'status.hidden_sidebar' => '侧栏',
    'status.hidden_ai' => 'AI',
    'status.hidden_terminal' => '终端',
    'status.term_max'  => '终端最大化',
    'status.copied'     => '已复制 {n} 字符',
    'status.copied_msg' => '已复制消息',
    'status.pasted'      => '已粘贴 {n} 字符',
    'status.paste_pending' => '正在读取剪贴板…',

    'ai.input_hint'    => '（回车发送）',

    'editor.no_file'   => '（未打开文件 — 在资源管理器选中文件后按 Enter 打开）',
    'editor.readonly'  => '（只读）',
    'editor.too_large' => '文件过大，无法编辑：{size} 字节（上限 {limit}）',
    'editor.binary'    => '二进制文件 — 不可编辑',
    'editor.missing'    => '文件不存在：{path}',
    'editor.unreadable' => '无读取权限，无法打开：{path}',
    'editor.saved'     => '已保存',
    'editor.save_failed' => '保存失败：{msg}',
    'editor.save_no_permission' => '保存失败：权限不足或路径不可写',
    'editor.open'      => '打开',
    'editor.untitled'  => '未命名',

    'confirm.quit_dirty'  => '有未保存改动：y 退出 / n 取消',
    'confirm.close_dirty' => '该文件未保存：y 关闭丢弃 / n 取消',
    'confirm.discard'     => '丢弃工作区改动（不可恢复！）：y 确认丢弃 / n 取消：{path}',

    'term.running'       => '运行中',
    'term.exit'          => '退出码 {code}',
    'term.killed'        => '已中断 (SIGKILL)',
    'term.busy'          => '有命令正在运行，等它结束或按 Ctrl+C 中断',
    'term.spawn_failed'  => '无法启动命令进程',
    'term.empty'         => '（聚焦此面板后可输入命令，回车执行；↑/↓ 翻历史，Ctrl+L 清屏）',
    'term.interactive'   => '交互终端',
    'term.captured'      => '捕获中',
    'term.interactive_enter' => '在此输入即接管键盘（Esc 退出程序）',
    'term.focus_to_start'    => '（聚焦此面板即启动交互式 shell）',
    'term.interactive_exit'  => '交互终端已退出 · F2 可重新进入',
    'term.interactive_hint'  => '交互终端 · F2 退出捕获',

    'git.not_repo'       => '（当前目录不是 git 仓库）',
    'git.view_status'    => '状态 {n}',
    'git.view_log'       => '日志 {n}',
    'git.loading'        => '（加载中…）',
    'git.clean'          => '（工作区干净）',
    'git.no_log'         => '（无提交历史）',
    'git.no_diff'        => '该文件无 diff',
    'git.commit_title'   => '提交信息',
    'git.msg_placeholder'=> '输入提交信息…',
    'git.changes'        => '变更 {n}',
    'git.log_title'      => '日志 {n}',
    'git.stage_all'      => '已暂存全部改动',
    'git.stage_file'     => '已暂存：{path}',
    'git.unstage_file'   => '已取消暂存：{path}',
    'git.unstage_all'    => '已取消全部暂存',
    'git.discard_file'   => '已丢弃工作区改动：{path}',
    'git.committed'      => '已提交',
    'git.commit_empty'   => '提交信息为空，已取消',
    'git.push_done'      => '已推送',
    'git.push_fail'      => '推送失败：{msg}',
    'git.op_fail'        => '操作失败：{msg}',
    'git.branch_pick'    => '切换分支',
    'git.no_branch'      => '（无其它分支）',
    'git.switch_done'    => '已切换到分支：{branch}',
    'git.switch_fail'    => '切换分支失败：{msg}',
    'git.action_commit'       => '提交',
    'git.action_commit_all'   => '提交变更',
    'git.action_commit_push'  => '提交和推送',
    'git.action_commit_sync'  => '提交和同步',
    'git.hint'           => '[a]全暂存 [s]暂存 [c]提交 [p]推送 [L]日志 [Enter]diff [R]刷新',
    'search.placeholder'      => '搜索文件内容…',
    'search.hint'             => '按回车搜索…',
    'search.prompt'           => ' > ',
    'search.status_searching' => '搜索中…',
    'search.status_no_results' => '无匹配结果',
    'search.results'          => '{n} 个匹配 / {files} 个文件',
    'search.status_error'     => '搜索出错：{msg}',
    'search.truncated'        => '已截断（前 {n} 条），请缩小范围',
    'search.empty_query'      => '请输入搜索关键词',

    'ai.no_provider'     => '未配置任何 Provider',
    'ai.no_key'          => '缺少 API key：请设置环境变量 {env}',
    'ai.no_selection'    => '未选模型 · Ctrl+P 选 Provider、Ctrl+N 选模型',
    'ai.caps'            => '能力',
    'ai.caps_none'       => '未声明任何能力',
    'ai.streaming'       => '生成中…',
    'ai.cancelled'       => '已停止生成',
    'ai.max_steps'       => '已达工具调用步数上限（{steps}），停止继续执行',
    'ai.await_approval'  => 'AI 请求执行工具，y 允许 / n 拒绝',
    'ai.attach_missing' => '@{name}：文件不存在或越出项目根，已忽略',
    'ai.attached' => '已附加 {n} 字节代码到输入框',
    'ai.attach_empty' => '选区为空，未附加',
    'ai.attach_no_selection' => '先在编辑器里拖选文本，再附加',
    'ai.attach_no_file' => '当前没有打开的文件',
    'menu.ai' => 'AI',
    'ai.act.explain' => '解释这段代码',
    'ai.act.comment' => '为这段代码加注释',
    'ai.act.refactor' => '给出重构建议',
    'ai.act.unittest' => '编写单元测试',
    'ai.attach_selection_label' => '附加编辑器选区',
    'ai.attach_file_label' => '附加当前文件',
    'ai.tool_mode_confirm' => '工具模式：切换为逐次确认',
    'ai.tool_mode_auto' => '工具模式：切换为自动执行',
    'ai.compact_now_label' => '立即压缩对话历史',
    'ai.providers_label' => '编辑模型配置…',
    'ai.providers_reloaded' => '模型配置已重载',
    'ai.providers_bad' => '模型配置未生效（{kind}），已沿用原配置',
    'ai.strategy_auto_item' => '自动选档（按任务类型）',
    'ai.strategy_auto_on' => '已切回自动选档：按任务类型自动挑档（快捷动作与 /前缀）',
    'ai.strategy_applied_pinned' => '已切到策略「{strategy}」→ {provider}/{model}（已钉住，自动选档暂停）',
    'ai.kind_unknown' => '未知任务类型「/{kind}」（已知：{known}）——本条未发送；想发字面量请写 //',
    'ai.kind_ignored_pinned' => '本条带了任务类型 /{kind}，但当前是手动选档状态，自动选档未生效（Ctrl+R 可切到「自动」）',
    'ai.strategy_item' => '策略：{label}',
    'ai.strategy_applied' => '已切到策略「{strategy}」→ {provider}/{model}',
    'ai.strategy_unknown' => '没有名为「{strategy}」的策略',
    'ai.strategy_no_provider' => '策略「{strategy}」的目标 provider「{provider}」未配置',
    'ai.strategy_no_model' => '策略「{strategy}」的模型「{model}」未在 provider「{provider}」里声明（拒绝静默回退）',
    'ai.strategy_missing_caps' => '策略「{strategy}」要求 {caps} 能力，但 {model} 没声明——已拒绝切换（否则该能力会静默失效）',
    'ai.strategy_none' => '没有配置任何模型策略（可在模型配置文件的 @strategies 段里添加）',
    'ai.strategy_offpeak_item' => '折扣中',
    'ai.strategy_offpeak_applied' => '折扣时段：已自动切到策略「{strategy}」→ {provider}/{model}（折扣至 {until}）',
    'ai.strategy_offpeak_all_day' => '全天',
    'ai.cleared_menu' => '清空对话',
    'ai.cleared' => '对话已清空',
    'ai.compact_started' => '正在压缩对话历史…',
    'ai.compact_failed' => '压缩失败，已保留原对话',
    'ai.compact_done' => '对话已压缩（{before} → {after} 条）',
    'ai.compact_instruction' => "请把下面这段对话历史压缩成一份简明的中文摘要，供后续对话作为背景使用。\n要求：保留用户的意图与约束、已达成的结论与决定、涉及的文件路径与关键代码位置、未完成的事项；舍弃寒暄与重复内容。只输出摘要正文，不要复述本指令。\n\n===== 待压缩的对话历史 =====\n{history}",
    'help.e_ai_explain' => 'AI 解释代码（编辑器内）',
    'ai.error'           => 'AI 请求失败：{msg}',
    'ai.http_error'      => 'HTTP {code}',
    'ai.empty'           => '（模型没有返回内容）',
    'ai.busy'            => '正在生成中，无法同时发起新请求',
    'ai.switched'        => '已切换到 {provider} / {model}',
    'ai.clear_hint'      => '（Ctrl+L 清空对话）',

    // ── M6 R2 帮助页 ─────────────────────────────
    'help.group_global'    => '全局',
    'help.group_editor'    => '编辑器',
    'help.group_terminal'  => '终端',
    'help.group_explorer'  => '资源管理器',
    'help.group_git'       => 'GIT',
    'help.group_search'    => '搜索',
    'help.group_ai'        => 'AI 对话',
    'help.g_quit'          => '退出（有未保存改动先确认）',
    'help.g_tab'           => '切焦点；在输入框/编辑器里=缩进 4 空格、有候选时接受补全',
    'help.g_tab_back'      => '反向切焦点；在输入框/编辑器里=反向缩进、有候选时上一个候选',
    'help.g_add_cursor'    => '在上一行/下一行加一个编辑光标（多行同时编辑）；Esc 取消多光标',
    'help.g_add_cursor_click' => '在点击处加一个编辑光标（不产生选区；不按 Alt 就是普通拖选复制）',
    'help.g_esc'           => '退出；若面板自身有语义则先由面板处理',
    'help.g_q'             => '非输入态退出',
    'help.g_help'          => '打开/关闭本帮助',
    'help.g_menu'           => '打开顶部菜单栏',
    'help.g_theme'           => '切换配色主题',
    'help.g_wheel'         => '滚动当前面板',
    'help.g_wheel_h'       => '横向滚动当前面板',
    'help.g_click'         => '切换焦点 / 选中条目',
    'help.e_move'          => '移动光标（行首行尾会跨行）',
    'help.e_home_end'      => '行首 / 行尾',
    'help.e_page'          => '翻页',
    'help.e_indent'        => '缩进 4 空格 / 反向缩进（多光标时作用于全部光标）',
    'help.e_newline'       => '换行',
    'help.e_del'           => '删除前一个 / 后一个字符',
    'help.e_save'          => '保存',
    'help.e_close'         => '关闭当前文件',
    'help.e_switch'        => '在已打开文件间切换',
    'help.e_type'          => '插入字符',
    'help.e_autopair'      => '自动配对：打左补右、跳过已有右符号、空对退格删两个、选中包裹',
    'help.t_run'           => '执行命令',
    'help.t_history'       => '命令历史',
    'help.t_move'          => '移动光标',
    'help.t_home_end'      => '行首 / 行尾',
    'help.t_del'           => '删除前一个 / 后一个字符',
    'help.t_page'          => '滚动回看输出',
    'help.t_clear'         => '清屏',
    'help.t_cancel'        => '中断正在跑的命令',
    'help.t_interactive'   => 'F2 切换终端键盘捕获（交互 shell 是默认模式）',
    'help.x_move'          => '移动选中',
    'help.x_nav'           => '展开（→ 进入子项）/ 折叠（← 回父级）',
    'help.x_enter'         => '展开折叠目录 或 打开文件',
    'help.x_open'          => '打开文件',
    'help.x_arrow'         => '展开 / 折叠',
    'help.x_dblclick'      => '展开 / 折叠',
    'help.gi_type'         => '输入提交信息',
    'help.gi_commit'       => '提交',
    'help.gi_backspace'    => '删除提交信息末字符',
    'help.gi_move'         => '移动选中',
    'help.gi_stage'        => '暂存选中文件',
    'help.gi_unstage'      => '取消暂存选中文件',
    'help.gi_esc'          => '关闭下拉菜单',
    'help.gi_icons'        => '暂存 / 取消暂存 / 丢弃改动 / 查看 diff',
    'help.s_type'          => '输入搜索关键词',
    'help.s_enter'         => '搜索 / 打开选中命中',
    'help.s_backspace'     => '删除关键词末字符',
    'help.s_move'          => '在结果间移动',
    'help.a_type'          => '输入消息',
    'help.a_at_ref'        => '引用项目内文件（发送前展开为带高亮的代码块）',
    'help.a_complete'      => '有候选时接受补全 / 选上一个；无候选时插 4 空格缩进',
    'help.a_send'          => '发送',
    'help.a_provider'      => '切换 Provider',
    'help.a_model'         => '切换模型',
    'help.a_strategy'      => '切换模型策略',
    'help.a_clear'         => '清空对话',
    'help.a_history'       => '调取历史提问',
    'help.a_page'          => '滚动消息流',
    'help.a_stop'          => '停止生成',
    'help.e_paste'         => '粘贴（Ctrl+V / Shift+Ins）',
    'help.title'           => '快捷键',
    'help.close'           => 'Esc / ? / q 关闭',
    'help.scroll_hint'     => '↑↓ 或滚轮滚动',

    // ── 顶部菜单栏（Backlog 接入）────────────────
    'menu.file'              => '文件',
    'menu.file_open'         => '打开',
    'menu.file_save'         => '保存',
    'menu.file_close'        => '关闭',
    'menu.file_quit'         => '退出',
    'menu.view'              => '视图',
    'menu.view_theme'        => '切换主题',
    'menu.view_focus_editor' => '焦点 → 编辑器',
    'menu.view_focus_terminal' => '焦点 → 终端',
    'menu.view_focus_explorer' => '焦点 → 资源管理器',
    'menu.view_focus_ai'     => '焦点 → AI 对话',
    'menu.view_lang'         => '切换语言',
    'menu.view_sidebar_hide' => '隐藏侧栏',
    'menu.view_sidebar_show' => '显示侧栏',
    'menu.view_ai_hide'      => '隐藏 AI 栏',
    'menu.view_ai_show'      => '显示 AI 栏',
    'menu.view_term_hide'    => '隐藏终端',
    'menu.view_term_show'    => '显示终端',
    'menu.view_term_max'     => '最大化终端',
    'menu.view_term_restore' => '还原终端',
    'menu.terminal'          => '终端',
    'menu.term_cancel'       => '中断命令',
    'menu.term_clear'        => '清屏',
    'menu.help'              => '帮助',
    'menu.help_shortcuts'    => '快捷键',
    'menu.help_about'        => '关于',
    'menu.file_plugins'      => '已安装插件...',
    'menu.plugins' => '插件',
    'plugins.commands' => '命令',
    'plugins.command_failed' => '插件 {plugin} 的命令 {cmd} 执行失败',
    'plugins.shortcut_reserved' => '快捷键 {key} 与系统快捷键冲突，已忽略（仍可从「插件」菜单触发）',
    'plugins.shortcut_taken' => '快捷键 {key} 已被 {owner} 占用，已忽略',
    'plugins.shortcut_unsupported' => '快捷键 {key} 不支持（仅支持 Ctrl+字母 与 F1–F12）',
    'plugins.no_executor' => '插件声明了命令但未实现 executeCommand()，命令不可用',
    'plugins.command_unavailable' => '命令不可用（{reason}）',
    'plugins.title'          => '已安装插件',
    'plugins.enabled'        => '已启用',
    'plugins.disabled'       => '已禁用',
    'plugins.toggled_on'     => '插件 {id} 已启用',
    'plugins.toggled_off'    => '插件 {id} 已禁用',
    'plugins.no_config'      => '无可配置项',
    'plugins.no_plugins'     => '(无插件)',
    'plugins.config_path'    => '插件配置文件',
    'plugins.hint'          => 'Space 切换选中插件的启用/禁用 · 按 Enter 用 ViceCode 自带的编辑器打开上面文件直接改（Ctrl+S 保存即生效）',
    'plugins.close'          => 'Esc / q 关闭',
    'plugins.reloaded'       => '插件配置已重新加载',
    'about.title'            => '关于 ViceCode',
    'about.line1'            => '基于 php-tui 的终端 IDE',
    'about.line2'            => 'Swoole 协程驱动 · 多 Provider LLM',
    'about.line3'            => 'F10 打开菜单 · ? 查看快捷键 · Ctrl+Q 退出',
    'status.lang'            => '语言',

    // 主题名（状态栏 / 菜单显示，跟随语言包）
    'theme.dark'             => '深色',
    'theme.midnight'         => '午夜蓝',

    // 模型能力名（config/providers.php 的 capabilities；未知能力按原文显示）
    'cap.tools'              => '工具调用',
    'cap.reasoning'          => '推理',
    'cap.vision'             => '识图',
    'cap.audio'              => '语音',

    // 帮助页「触发方式」列（鼠标 / 输入描述）
    'keys.wheel'             => '滚轮',
    'keys.hwheel'            => '横向滚轮',
    'keys.click'             => '单击',
    'keys.alt_click'         => 'Alt+点击',
    'keys.printable'         => '可打印字符',
    'keys.click_file'        => '单击文件',
    'keys.click_triangle'    => '单击行首三角',
    'keys.dblclick_dir'      => '双击目录',
    'keys.click_icon'        => '点击图标',

    // 命令面板（F1）
    'palette.title'          => '命令面板',
    'palette.placeholder'    => '输入以筛选命令…',
    'menu.command_palette'   => '命令面板',
    'help.g_palette'         => '打开命令面板（F1）',

    // 自定义面板（插件浮层）
    'menu.plugin_panels'     => '插件面板',
    'panel.title'            => '插件面板',
    'panel.empty'            => '（无插件面板）',
    'panel.close'            => 'Esc 关闭 · Tab 切换',
];
