...
> 浮层是模态的：打开期间 `Esc` / `q` 关闭，`↑/↓` 移动选中插件、`Space` 切换其启用/禁用、翻页键滚动，`Enter` 在 ViceCode 编辑器内打开**插件专用**配置文件。
...
**状态写在哪**：插件专用配置文件 `~/.vicecode.plugins.json` 里该插件的 `enabled` 键：

```json
{
    "clock": { "enabled": false, "timezone": "Asia/Shanghai" }
}
```

- **缺省即启用**：没写 `enabled`（或写 `true`）就是启用，老配置零迁移。只有显式 `false` / `0` / `"0"` 才算禁用。
- **`enabled` 是核心保留键**：归核心管，**不会**传给你的 `configure()`，也不会出现在插件页展示的「有效配置」里——你只管自己的业务配置键，不必处理它。
- 切换**只动 `enabled` 一个键**，该插件其它配置键与其它插件的配置都原样保留。

**怎么切**（两条入口，等价）：

- 侧栏「扩展」tab：`↑/↓` 选中插件，按 **`Space`** 切换。
- 「文件 → 已安装插件...」浮层：`↑/↓` 选中，按 **`Space`** 切换；`Enter` 仍是打开配置文件。

**生效时机：立即热生效。** 切换会当场重算「启用的插件子集」并重建命令 / 菜单 / 自定义面板注册表，不重新加载插件实例（插件自身的运行期状态得以保留）。你也可以直接在配置文件里改 `enabled` 再 `Ctrl+S`——那是**同一条生效路径**（都走「重读配置 → 重算启用集 → 重建注册表」）。

**被禁用意味着什么**：该插件

- 不再向状态栏注入任何段（`statusSegments()` 根本不会被调用）；
- 不再参与周期重绘（`tickInterval()` 被忽略）；
- 命令从菜单与命令面板消失、快捷键解绑、状态栏段不再可点；
- 自定义面板（`panels()`）从浮层消失；
- 不再接收生命周期事件（`onEvent()`）。

但它**仍然列在**插件页与侧栏「扩展」tab 里（标注 `[已禁用]`），否则禁用以后就再也找不到、没法恢复——这也是刻意选择。

### 3.12 补全候选（V1.2，可选）

插件可以为**输入上下文**提供 Tab 补全候选。这是「插件将来接 AI 自动补全与提示」的落点。

```php
public function completions(string $context, string $text, int $cursor, string $prefix): array
{
    // 只对 AI 输入框、且正在输入 `/` 开头的指令前缀时给候选
    if ($context !== 'ai_input' || !str_starts_with($prefix, '/')) {
        return [];
    }
    return [
        new \App\Core\CompletionItem('/plan', '/plan ', '切到计划档'),
    ];
}
```
...
| 参数 | 含义 |
|---|---|
| `$context` | 输入上下文：`ai_input` / `editor` / `search` / `commit` |
| `$text` | 该输入框当前的完整文本 |
| `$cursor` | 光标位置的**字符**下标（AI 输入框 / 搜索框 / 提交框没有光标位，一律等于文本长度） |
| `$prefix` | 光标前正在输入的那段**非空白 token**（空串 = 光标前是空白/行首） |

> ⚠️ **核心不判触发符**：`@` 只是内建文件补全自己的约定，你的插件可以认任何前缀
> （例如 `/` 指令前缀）。认不认由你按 `$prefix` 自己决定。

**返回值**：`list<CompletionItem>`，每条 = `new CompletionItem(label, insert, detail?)`。
`label` 是列表里显示的、`insert` 是接受后**写回文本**的内容（替换掉 `$prefix` 那段）。
返回 `[]`（或干脆不实现这个方法）就没有候选。

**按键由核心独占，插件只提供数据**：

- **有候选**：`Tab` = 接受当前候选、`Shift+Tab` = 上一个候选；
- **无候选**：多行上下文（编辑器 / AI 输入框）`Tab` = 缩进 4 空格、`Shift+Tab` = 反向缩进；
  单行输入（搜索框 / 提交框）与其它面板 `Tab` 仍切焦点；
- `Esc` 在候选打开时只关候选，不会退出程序。

插件**拿不到也不该拿按键** —— 这样插件不可能把全局键玩坏，也不必关心候选怎么画
（与「声明数据 + 核心渲染」的既有取向一致：渲染成什么样、怎么滚动、按键怎么走都是核心的事）。

**两个约束**：

- **同步调用**：核心在**每次按键之后**重算一次候选，实现必须快。
  将来要接 AI 自动补全这类慢查询，需要给 provider 补「pending / 流式」语义（尚未设计）。
- **失败只影响自己**：provider 抛异常或返回脏类型时核心会跳过它（空候选），
  不影响打字、也不影响其它插件 —— 与 Loader「单个插件失败只跳过它」同一取向。

**禁用即失去候选**：与其它能力同一条启停链路（整体重建 provider 表、只遍历启用子集），
`Space` 一关，下一次重建就不含它。
...
```bash
# 1) headless 单测（无需终端）：加载、坏插件跳过、段聚合、窄屏裁剪、启用/禁用开关
php tests/plugin_unit.php
php tests/plugin_enabled_unit.php

# 2) 真实 pty 验收（需支持伪终端的环境）：验证状态栏真正渲染出插件内容、开关真的按得动
php tests/pty_plugin.php
php tests/pty_plugin_toggle.php
```
...
- 启用/禁用开关：`~/.vicecode.plugins.json` 的 `<id>.enabled`（核心保留键，缺省启用）；侧栏「扩展」tab 与插件页浮层里选中后按 `Space` 切换，**立即热生效**（§3.11）。

**V1.2 已支持**：
- 补全候选：`completions()` 为 AI 输入框 / 编辑器 / 搜索框 / 提交框提供 Tab 补全候选（§3.12）。按键由核心独占、插件只给数据；内建的 `@文件` 路径补全与插件候选走**同一条路**，所以这个扩展点不是"预留接口"而是已经被真实功能用着。

**仍未支持（设计取舍）**：暂无。路线图以产品需求为准。
